...
//
//   2) Live latest N blocks from "now":
//      node export_mev_data.js blocksLive <count>
//
//   3) Live mempool snapshots (stop after ~maxBlocks new blocks):
//      node export_mev_data.js mempool <intervalSeconds> [maxBlocks]
//
//      e.g., node export_mev_data.js mempool 5 1000
//
...
async function exportSingleBlock(blockNumber, txFile, blockFile) {
  ...
  for (let i = 0; i < txs.length; i++) {
    ...
    const maxPriorityFeePerGas = tx.maxPriorityFeePerGas
      ? hexToDec(tx.maxPriorityFeePerGas)
      : "";
    ...
    const receiptStatus =
      receipt && typeof receipt.status !== "undefined" ? receipt.status : "";
    const gasUsedTx =
...
// ---------- MODE 1: Historical blocks <fromBlock> <toBlock> ----------
...
async function exportBlocksLive(count) {
  ...
  while (collected < count) {
    ...
    if (latest <= lastExported) {
      await sleep(5000); // wait 5s for new blocks
      continue;
    }
...
// ---------- MODE 2: Mempool snapshots: mempool <intervalSec> [maxBlocks] ----------

async function snapshotMempoolAndNetwork(intervalSec, maxBlocks) {
  ...
  if (maxBlocks && maxBlocks > 0) {
    ...
    console.error(
      `[*] Mempool snapshots will stop after ~${maxBlocks} new blocks (starting from block ${startBlock})`,
    );
  }

  console.error(
    `[*] Taking mempool+peer snapshots every ${intervalSec}s. Press Ctrl+C to stop earlier.`,
  );
  ...
  async function oneSnapshot() {
    ...
    // ---- Stop condition based on block count ----
    if (startBlock !== null && maxBlocks && maxBlocks > 0) {
      const latest = await provider.getBlockNumber();
      const delta = latest - startBlock;
      console.error(`[*] Blocks since start: ${delta}/${maxBlocks}`);

      if (delta >= maxBlocks) {
        console.error("[*] Reached maxBlocks threshold. Stopping mempool snapshots.");
        clearInterval(timer);
        mpFile.end();
        peerFile.end();
        process.exit(0);
  ...
  // First snapshot immediately, then repeat
  await oneSnapshot();
  timer = setInterval(oneSnapshot, intervalSec * 1000);
}
...
async function main() {
  ...
  if (mode === "blocks") {
    ...
    const maxBlocks = arg2 ? parseInt(arg2, 10) : null;

    if (Number.isNaN(intervalSec) || intervalSec <= 0) {
      console.error(
        "Usage: node export_mev_data.js mempool <intervalSeconds> [maxBlocks]",
      );
      process.exit(1);
    }
    if (arg2 && (Number.isNaN(maxBlocks) || maxBlocks <= 0)) {
      console.error("Invalid maxBlocks value.");
      process.exit(1);
    }

    await snapshotMempoolAndNetwork(intervalSec, maxBlocks);

  } else {
    console.error("Usage:");
    console.error("  node export_mev_data.js blocks <fromBlock> <toBlock>");
    console.error("  node export_mev_data.js blocksLive <count>");
    console.error("  node export_mev_data.js mempool <intervalSeconds> [maxBlocks]");
    process.exit(1);
  }
}
